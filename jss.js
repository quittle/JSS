 ...
 * Website: http://www.dustindoloff.com/projects/jss
 * Date: 7 / 31 / 2014
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
...
function createStyle(el, url){
	...
	xmlhttp.open("GET", url, true); // Make an asynchronous request (like all CSS requests)
	xmlhttp.send();
}
...
/* Grab the links and generate CSS on window load */
w.addEventListener("load", function(){
	// Grab all JSS links
	var links;
	if(d.querySelectorAll){
		links = d.querySelectorAll("link[rel=" + REL_VALUE + "]");
	}else{
	...
	// Loop through them and create a style element for each link with generated CSS
	for(var i = 0, l = links.length; i < l; i++){
		var link = links[i],
			url = link.href;

		if(url != null){
			createStyle(link, url);
		}
	}
});